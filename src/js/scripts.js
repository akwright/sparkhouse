    ...
    polyBG: {
      ...
      init: function() {
        ...
        function refresh(ele) {
            randomize();
            for(var i = 0; i < document.querySelector('#welcome svg').childNodes.length; i++) {
                var polygon = document.querySelector('#welcome svg').childNodes[i];
        ...
        function onResize() {
            document.querySelector('#welcome svg').remove();
    ...
    setupMobileScroll: {
      ...
      init: function() {
        ...
        var imageWrap   = document.querySelector('.service-image-wrap'),
            ...
            houseImages = Array.prototype.slice.call(document.querySelectorAll('.service-image img')),
            ...
            houseHeight = houseImgOne.clientHeight || houseImgOne.offsetHeight,
            ...
            didScroll   = false,
            SVGMain     = document.getElementById('house_blueprint'),
        ...
        setTimeout(function( imageWrap ) {
          return function() {
            if( window.scrollY >= (iwOffsetTop - ( (docElem.clientHeight || docElem.offsetHeight) / 2)) && window.scrollY <= ( main.offsetTop - iwOffsetTop) ) {
        ...
        var mobileScrollHandler = function() {
          ...
          scrollPage = function() {
            ...
            //
            if ( window.scrollY >= textBlock2.offsetTop && window.scrollY <= (textBlock2.offsetTop + (textBlock2.clientHeight || textBlock2.offsetHeight) + (docElem.clientHeight || docElem.offsetHeight)) ) {
              textWrap.classList.add('has-overlay');
            } else if ( window.scrollY >= textBlock3.offsetTop && window.scrollY <= (textBlock3.offsetTop + (textBlock3.clientHeight || textBlock3.offsetHeight) + (docElem.clientHeight || docElem.offsetHeight)) ) {
              textWrap.classList.add('has-overlay');
            } else if ( window.scrollY >= textBlock4.offsetTop && window.scrollY <= (textBlock4.offsetTop + (textBlock4.clientHeight || textBlock4.offsetHeight) + (docElem.clientHeight || docElem.offsetHeight)) ) {
              textWrap.classList.add('has-overlay');
            } else {
            ...
            if ( window.scrollY >= (textBlock1.offsetTop + (textBlock1.clientHeight || textBlock1.offsetHeight) + (docElem.clientHeight || docElem.offsetHeight)) ) {
              // Show the blueprint
              showImage(imageEle);
            }

            // If we're past the develop block
            if ( window.scrollY >= (textBlock2.offsetTop + (textBlock2.clientHeight || textBlock2.offsetHeight) + (docElem.clientHeight || docElem.offsetHeight)) ) {
              // Animate the house drawing
              animateOffsets();
            }

            // Once we're past the market block
            if ( window.scrollY >= (textBlock3.offsetTop + (textBlock3.clientHeight || textBlock3.offsetHeight) + (docElem.clientHeight || docElem.offsetHeight) + scrollPad) ) {
              // Fade in the image
              showImage(houseImgOne);
            }

            // Once we get past the iterate block
            if ( window.scrollY >= (textBlock4.offsetTop + (textBlock4.clientHeight || textBlock4.offsetHeight) + (docElem.clientHeight || docElem.offsetHeight) + scrollPad) ) {
        ...
        var mobileImageResize = function() {
          houseHeight = houseImgOne.clientHeight || houseImgOne.offsetHeight;
          
          houseImgOne.style.top = (( (SVGMain.clientHeight || SVGMain.offsetHeight) - houseHeight) / 2) +'px';
          houseImgTwo.style.top = (( (SVGMain.clientHeight || SVGMain.offsetHeight) - houseHeight) / 2) +'px';
        };

        mobileImageResize(houseImages);

        window.addEventListener( 'scroll', mobileScrollHandler, false );
        window.addEventListener( 'resize', mobileImageResize, false );
